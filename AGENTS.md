...
5. **Supported files** — see `sage/config.py` + `sage/loaders.py`. Use `is_env_file()` / `is_supported_file()` (file named `.env` has empty `Path.suffix`).
6. **Clear Search** — clear widget state via a flag before the text_area is created (never mutate `search_query` after the widget instantiates).
7. **Chroma docs may be null** — coerce hit text before `.strip()`.
