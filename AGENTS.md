...
- **Supported formats** include `.yaml`/`.yml`, `.env` / `.env.*`, and `.log` (see `sage/config.py` + `sage/loaders.py`). All processing is local-only; never add cloud upload paths for these files.
